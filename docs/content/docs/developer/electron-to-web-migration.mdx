...
Thankfully, Electron ships web UI so most of our React/TailwindCSS styling was in-tact. Though many of the server communication had to be mapped from Electron IPC to Client-Server. We replaced those with tRPC to preserve type safety.
...
```
Previously, we were storing most user/project information on the users machine as serialized JSON. Now, we store them in Postgres tables in Supabase. We maintain a caching layer on top of posgres on the browser for ease of access and better loading. 
