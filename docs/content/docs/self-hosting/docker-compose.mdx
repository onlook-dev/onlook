...
For Docker Compose deployment, create a production environment file:

```bash
# Install dependencies first
bun install

# Copy your existing .env to .env.production for Docker
cp .env .env.production
```

Or run the interactive setup script and then copy:
