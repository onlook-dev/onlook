 ...
 */
    async messageAndGetFinalResponse(message: string, editorEngine: EditorEngine, onToolCall: OnToolCallHandler): Promise<ChatMessage> {
        ...
        return new Promise<ChatMessage>(async (resolve, reject) => {
            try {
                const { addToolResult, sendMessage, status } = new OnlookChat({
                    ...
                    onToolCall: (toolCall) => { onToolCall(this.agentType, addToolResult)(toolCall) },
                    onFinish: (options) => {
                        ...
                        console.log("OnlookChat - Is incomplete:", isIncomplete, status);
                        ...
                        if (!isIncomplete && lastPartComplete) {
                            ...
                            console.log("OnlookChat - Resolving with final message", status);
                            resolve(message as ChatMessage);
                        } else {
                            console.log("OnlookChat - Message is incomplete, waiting for completion", status);
                        }
