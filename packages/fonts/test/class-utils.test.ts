...
describe('handleStringLiteralClassName', () => {
    ...
    const casesDir = path.resolve(__dirname, 'data/string-literal-classname');
    ...
    for (const testCase of testCases) {
        test(`should handle case: ${testCase}`, async () => {
            ...
            const inputContent = await Bun.file(inputPath).text();
            const result = await processTestCase(inputContent, 'handleStringLiteralClassName');
...
describe('handleJSXExpressionClassName', () => {
    const SHOULD_UPDATE_EXPECTED = false;
    const casesDir = path.resolve(__dirname, 'data/jsx-expression-classname');
    ...
    for (const testCase of testCases) {
        test(`should handle case: ${testCase}`, async () => {
            ...
            const inputContent = await Bun.file(inputPath).text();
            const result = await processTestCase(inputContent, 'handleJSXExpressionClassName');
