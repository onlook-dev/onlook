...
import { CodesandboxClient } from '@/provider/e2b/index';
import { CodesandboxSandboxFile } from './index';
import { ClientError, ClientErrorCode, ClientOptions } from '@/provider/definition';
...
class CodesandboxMockClient extends CodesandboxClient {
...
describe('CodesandboxSandboxFile', () => {
    let mockClient: CodesandboxMockClient;
    let sandboxFile: CodesandboxSandboxFile;

    beforeEach(() => {
        mockClient = new CodesandboxMockClient({ sandboxId: 'test-sandbox-id' });
        sandboxFile = new CodesandboxSandboxFile(mockClient);
        jest.clearAllMocks();
    });

    describe('copy', () => {
        it('should throw Unimplemented error', async () => {
            await expect(sandboxFile.copy({ oldPath: '/old', newPath: '/new' })).rejects.toThrow(
    ...
    describe('delete', () => {
        it('should delete file successfully', async () => {
            ...
            expect(mockClient._sandbox.files.remove).toHaveBeenCalledWith('/test/file.txt');
        });
    ...
    describe('download', () => {
        it('should throw Unimplemented error', async () => {
            await expect(sandboxFile.download({ path: '/test/file.txt' })).rejects.toThrow(
                new ClientError(ClientErrorCode.Unimplemented, 'Not implemented', false),
            );
        });
    });
    ...
    describe('read', () => {
        it('should read file successfully', async () => {
            ...
            expect(mockClient._sandbox.files.read).toHaveBeenCalledWith('/test/file.txt');
    ...
    describe('stat', () => {
        it('should get file info successfully for file', async () => {
            ...
            (mockClient._sandbox.files.getInfo as any).mockResolvedValue(mockFileInfo);

            const input = { path: '/test/file.txt' };
            const result = await sandboxFile.stat(input);

            expect(mockClient._sandbox.files.getInfo).toHaveBeenCalledTimes(1);
            expect(mockClient._sandbox.files.getInfo).toHaveBeenCalledWith('/test/file.txt');
        ...
        it('should get file info successfully for directory', async () => {
            ...
            expect(mockClient._sandbox.files.getInfo).toHaveBeenCalledTimes(1);
            expect(mockClient._sandbox.files.getInfo).toHaveBeenCalledWith('/test/dir');
    ...
    describe('write', () => {
        ...
        it('should skip existing files when overwrite is false', async () => {
            ...
            expect(mockClient._sandbox.files.exists).toHaveBeenCalledWith('/test/file.txt');
            expect(mockClient._sandbox.files.write).toHaveBeenCalledTimes(1);
            expect(mockClient._sandbox.files.write).toHaveBeenCalledWith([]);
        });
