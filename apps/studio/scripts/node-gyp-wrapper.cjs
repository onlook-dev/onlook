...
const nodeDir = path.dirname(path.dirname(systemNodePath));
...
console.log('Using Node binary:', systemNodePath);
...
if (args[0] === 'npm') {
    ...
    if (npmArgs.includes('install') && npmArgs.includes('node-pty')) {
        const nodeGypProcess = spawn(systemNodePath, [nodeGypPath, 'rebuild'], {
            stdio: 'inherit',
            env,
        ...
        const npmProcess = spawn(systemNodePath, [require.resolve('npm/bin/npm-cli.js'), ...npmArgs], {
            stdio: 'inherit',
            env,
            shell: os.platform() === 'win32'
        });
    ...
    const nodeGypProcess = spawn(systemNodePath, [nodeGypPath, ...args], {
        stdio: 'inherit',
        env,
        shell: os.platform() === 'win32'
    });
