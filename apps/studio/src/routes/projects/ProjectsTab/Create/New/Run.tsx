...
const NewRunProject: StepComponent = ({ props, variant }) => {
    ...
    const runProjectCommand = getRunProjectCommand(projectData.folderPath || '');
    ...
    const renderContent = () => (
        <div className="flex flex-col gap-4 w-full">
            <div className="border-[0.5px] bg-background-onlook/50 rounded-lg p-4 flex flex-row gap-2 items-center relative">
                <code className="text-sm overflow-scroll text-nowrap pr-20">
                    {runProjectCommand}
                </code>
                ...
                >
                    <AnimatePresence mode="wait" initial={false}>
                        ...
                        >
                            {isCopied ? <Icons.Check /> : <Icons.ClipboardCopy />}
                        </motion.span>
                    </AnimatePresence>
                </Button>
            </div>
