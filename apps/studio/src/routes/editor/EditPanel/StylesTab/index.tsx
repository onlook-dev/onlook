...
const SingleInputRenderer = memo(({ style }: { style: SingleStyle }) => {
...
const CompoundRenderer = memo(({ style }: { style: CompoundStyleImpl }) => {
    if (
...
const ManualTab = observer(() => {
    ...
    const renderAccordianHeader = useCallback(
        (groupKey: string) => {
            return (
                <Tooltip>
                    <TooltipTrigger
                        asChild
                        disabled={editorEngine.style.mode !== StyleMode.Instance}
                    >
                        <div
                            className={cn(
        ...
        [editorEngine.style.mode],
    );

    const styleSections = useMemo(
        () =>
            Object.entries(STYLE_GROUP_MAPPING).map(([groupKey, baseElementStyles]) => (
