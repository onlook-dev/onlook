    ...
    fork: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            try {
                ...
                const provider = await getProvider({ sandboxId: sourceBranch.sandboxId });
            ...
            try {
                return await ctx.db.transaction(async (tx) => {
                    ...
                    const provider = await getProvider({
                        sandboxId: SandboxTemplates[Templates.EMPTY_NEXTJS].id,
                    });
                    const blankSandbox = await provider.createProject({
                        source: 'template',
