...
export const useFolderImages = (folder: FolderNode) => {
    ...
    const readImageContent = useCallback(
        async (imagePath: string): Promise<ImageContentData | null> => {
            return editorEngine.image.readImageContent(imagePath);
        },
        [editorEngine.image],
    );

    const readImagesContent = useCallback(
        async (imagePaths: string[]): Promise<ImageContentData[]> => {
            ...
            try {
                ...
                const errorMessage =
                    error instanceof Error ? error.message : 'Failed to read images content';
    ...
    const loadFolderImages = useCallback(
        async (currentFolder: FolderNode | null) => {
            if (!currentFolder) {
