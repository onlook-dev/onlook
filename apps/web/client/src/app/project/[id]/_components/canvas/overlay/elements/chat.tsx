...
export const OverlayChat = observer(
    ({ selectedEl, elementId }: { selectedEl: ClickRectState | null; elementId: string }) => {
        ...
        const { submitMessage, isWaiting } = useChatContext();
        ...
        const handleSubmit = async () => {
            ...
            const streamMessage = await editorEngine.chat.getEditMessage(messageToSend);
            if (!streamMessage) {
                console.error('No edit messages returned');
                return;
            }
            submitMessage(streamMessage, ChatType.EDIT);
            setInputState(DEFAULT_INPUT_STATE);
        };
