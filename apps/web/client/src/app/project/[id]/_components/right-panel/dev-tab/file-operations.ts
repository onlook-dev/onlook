...
export const createFileInSandbox = async (session: SandboxSession, filePath: string, content: string = '', sandboxManager: SandboxManager): Promise<void> => {
    try {
        if (!session) {
...
export const createFolderInSandbox = async (session: SandboxSession, folderPath: string, sandboxManager: SandboxManager): Promise<void> => {
    try {
        if (!session) {
            throw new Error('No sandbox session available');
        }
