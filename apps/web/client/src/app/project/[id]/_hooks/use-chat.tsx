...
export const ChatProvider = observer(({ children }: { children: React.ReactNode }) => {
        ...
        onFinish: ({ message }) => {
            ...
            if (finishReason !== 'tool-calls') {
                ...
                editorEngine.chat.conversation.addOrReplaceMessage(
                    toOnlookMessageFromVercel(message, currentConversationId ?? ''),
                );
        ...
        onError: (error) => {
            ...
            if (lastMessageRef.current) {
                const currentConversationId =
                    editorEngine.chat.conversation.current?.conversation.id;
                editorEngine.chat.conversation.addOrReplaceMessage(
                    toOnlookMessageFromVercel(lastMessageRef.current, currentConversationId ?? ''),
                );
                lastMessageRef.current = null;
            }
        },
    ...
    return (
        <ChatContext.Provider value={{ ...chat, sendMessageToChat }}>
            {children}
